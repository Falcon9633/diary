...
public interface TeacherDAO extends JpaRepository<Teacher, Integer> {
    ...
    @Query("from Teacher t left join fetch t.subjectSet where t.email=:email")
    Teacher findByEmailWithSubject(@Param("email") String email);
    ...
    @Query("from Teacher teacher " +
            "left join fetch teacher.scheduleSet schedules " +
            "where schedules.subject.id=:subjectId and schedules.band.id=:bandId")
    Set<Teacher> findAllBySubjectAndBand(@Param("subjectId") int subjectId, @Param("bandId") int bandId);
